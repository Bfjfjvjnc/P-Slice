...
class MiguelCard extends BackingCard
{
  ...
  public override function init():Void
  {
    ...
    scrollBack = new FlxBackdrop(Paths.image('freeplay/backingCards/miguel/lowerLoop'), X, 20);
    scrollBack.setPosition(0, 250);
    scrollBack.flipX = true;
    scrollBack.alpha = 0.39;
    scrollBack.velocity.x = 110;
    add(scrollBack);

    scrollLower = new FlxBackdrop(Paths.image('freeplay/backingCards/miguel/lowerLoop'), X, 20);
    scrollLower.setPosition(0, 456);
    scrollLower.velocity.x = -110;
    add(scrollLower);

    redBar = new FlxSprite(0, 239).loadGraphic(Paths.image('freeplay/backingCards/miguel/redBar'));
    redBar.blend = BlendMode.MULTIPLY;
    redBar.alpha = 0.7;
    add(redBar);
    ...
    scrollTop.frames = Paths.getSparrowAtlas('freeplay/backingCards/miguel/topLoop');
    scrollTop.animation.addByPrefix('uzi', 'gorra info', 24, false);
    scrollTop.animation.addByPrefix('sniper', 'arm info', 24, false);
    scrollTop.animation.addByPrefix('rocket launcher', 'micro info', 24, false);
    scrollTop.animation.addByPrefix('rifle', 'guantes info', 24, false);
    scrollTop.animation.addByPrefix('base', 'base', 24, false);
    scrollTop.animation.play('base');

    add(scrollTop);

    scrollMiddle = new FlxBackdrop(Paths.image('freeplay/backingCards/miguel/middleLoop'), X, 15);
    scrollMiddle.setPosition(0, 346);
    add(scrollMiddle);
    scrollMiddle.velocity.x = 220;

    glowDark = new FlxSprite(-300, 330).loadGraphic(Paths.image('freeplay/backingCards/miguel/glow'));
    glowDark.blend = BlendMode.MULTIPLY;
    add(glowDark);

    glow = new FlxSprite(-300, 330).loadGraphic(Paths.image('freeplay/backingCards/miguel/glow'));
  ...
  {
    FlxTween.color(pinkBack, 0.25, 0xFFD42727, 0xFFFFD0D5, {ease: FlxEase.quadOut});
