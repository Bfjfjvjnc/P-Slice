...
class TankmanCard extends BackingCard
{
  ...
  public override function init():Void
  {
    ...
    multiplyBar = new FlxSprite(-10, 440).loadGraphic(Paths.image('freeplay/backingCards/tankman/multiplyBar'));
    multiplyBar.blend = BlendMode.MULTIPLY;
    add(multiplyBar);

    lightLayer = new FlxSprite(-360, 230).loadGraphic(Paths.image('freeplay/backingCards/tankman/orange gradient'));
    lightLayer.blend = BlendMode.ADD;
    add(lightLayer);

    multiply1 = new FlxSprite(-15, -125).loadGraphic(Paths.image('freeplay/backingCards/tankman/red'));
    multiply1.blend = BlendMode.MULTIPLY;
    add(multiply1);

    multiply2 = new FlxSprite(-15, -125).loadGraphic(Paths.image('freeplay/backingCards/tankman/red'));
    multiply2.blend = BlendMode.MULTIPLY;
    add(multiply2);

    lightLayer2 = new FlxSprite(-360, 230).loadGraphic(Paths.image('freeplay/backingCards/tankman/orange gradient'));
    lightLayer2.blend = BlendMode.ADD;
    add(lightLayer2);

    yellow = new FlxSprite(0, 0).loadGraphic(Paths.image('freeplay/backingCards/tankman/yellow bg piece'));
    yellow.blend = BlendMode.MULTIPLY;
    add(yellow);

    lightLayer3 = new FlxSprite(-360, 290).loadGraphic(Paths.image('freeplay/backingCards/tankman/red gradient'));
